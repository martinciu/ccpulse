...
// The returned row is raw, unstyled glyphs; renderXLabels wraps it in the
// colorMuted dimStyle. As of #335 the bar-chart Y labels (overlayYLabel) and
// the quota Y ticks (overlayYTicks) are colorMuted too, so all three label
// rows read as one muted chrome layer; the bar-chart Y labels are additionally
